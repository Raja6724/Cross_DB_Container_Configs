...
module.exports = function requestHandler(req, res, next) {
  ...
  if (serviceTag && businessServiceUtils.getGrantType(req.internalUrl.route.credentials) === 'user_token'){
    ...
    const serviceName = req.destinationCredentials && req.destinationCredentials.uniqueServiceName && req.internalUrl.route.credentials && req.internalUrl.route.credentials.uniqueServiceName ? req.internalUrl.route.credentials.uniqueServiceName : serviceTag;
    callBackend(req, res, req.session.user.businessServices[serviceName].accessToken, next);
  } else {
