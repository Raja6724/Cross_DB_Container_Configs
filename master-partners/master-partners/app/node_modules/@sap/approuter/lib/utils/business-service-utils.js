...
function cacheBSDestinations(req, cb) {
  ...
  addDestinationKey(req); // req.destinationKey = "some-subscriber_289a5eda-c301-4f47-9d29-93addc2ba4fd"
  let destinationBSCredentials = getDestinationBSCredentials(req); // get from cache
...
function mergeHTMLRepoDestinations(destinationBSCredentials, html5RepoDestinations) {
  ...
  // If destinationBSCredentials pointing to XSUAA or IAS exist, add HTML5 Repo app-host destination information to them, else
  // create new destinationBSCredentials entry from the html5 repo destination
  // this case is relevant for shared IAS token (use login token also for service session) or
  // IAS token exchange based on IAS dependency name
  // In addition it is possible to handle endpoints (backend destinations) in html5 repo destinations
      ...
      destinationBSCredentials[sapCloudServiceKey] = {
        credentials: {
          'html5-apps-repo': {
            app_host_id: html5RepoDestinations[sapCloudServiceKey].appHostId,
            xsappname: html5RepoDestinations[sapCloudServiceKey].xsappname,
            endpoints: html5RepoDestinations[sapCloudServiceKey].endpoints
          },
          ias: {
            sharedIASToken: html5RepoDestinations[sapCloudServiceKey].IASDependencyName ? false : true,
            IASDependencyName: html5RepoDestinations[sapCloudServiceKey].IASDependencyName
          },
          serviceName: html5RepoDestinations[sapCloudServiceKey].serviceName,
          destinationName: html5RepoDestinations[sapCloudServiceKey].destinationName,
          destinationId: html5RepoDestinations[sapCloudServiceKey].destinationId
        }
      };
    }
  }
...
function collectHTMLRepoDestinations(destination, html5RepoDestinations, sapCloudServiceKey) {
  if (html5RepoDestinations[sapCloudServiceKey]) {
    ...
    html5RepoDestinations[sapCloudServiceKey] = {
      ...
      IASDependencyName: destination['HTML5.IASDependencyName'] || destination['IASDependencyName'],
      endpoints: destination['endpoints'] || destination['destinations'],
      serviceName: destination['sap.cloud.service'],
...
function collectXsuaaOrXsuaaAppHostDestinations(destination, destinationBSCredentials, serviceName, destinationXsappname, sapCloudServiceKey, tracer) {
  ...
  destinationBSCredentials[sapCloudServiceKey] = {
    credentials: {
      destinationName: destination.Name,
      serviceName: serviceName, // <bs name>
      uniqueServiceName: sapCloudServiceKey, // <bs name>!<destination guid> or <bs name>
      destinationXsappname: destinationXsappname && destinationXsappname.valueOf()
    }
  };
